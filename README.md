# AI
صفحه در حال به روز رسانی می باشد


بدافزار :

===>>>  wormgpt  <<<===

===>>>  wolfgpt

===>>>  ghostgpt


==============================

دستیار هوش مصنوعی جدید شرکت Proton است — یک پلتفرم چت ایمن و با تمرکز بر حفظ حریم خصوصی

[ lumo proton ](https://lumo.proton.me/guest)

==============================

## compare ai

[ artificialanalysis ](https://artificialanalysis.ai/)

[ lmarena ](https://lmarena.ai/)

---
🔻ابزارهای هوش مصنوعی رایگان در برابر پولی!                                               
و این هم لینک ابزار های رایگان:
1. DeepSeek
https://www.deepseek.com

چین دوباره یه مدل اوپن سورس برگ ریزون  
دیگه معرفی کرده! در حد Grok 4 عمل می‌کنه، شرکت های غول آمریکایی خواب راحت ندارن !

تست ژیپو :
chat.z.ai
توضیحات بیشتر :
z.ai/blog/glm-4.5

chatcraft.org

2. Leonardo AI
https://www.leonardo.ai

3. Speechma
https://www.speechma.com

4. Gamma
https://www.gamma.app

5. Microsoft Designer
https://www.designer.microsoft.com

6. Suno AI
https://www.suno.ai

==============================

===>>> [agent](https://agent.ai/) <<<=== ===>>> job <<<===



پلتفرهای خارجی :



===>>> [ ai student ](https://one.google.com/join/ai-student) <<<=== ===>>> pro

===>>> [ felo ](https://felo.ai) <<<===

===>>> [ kimi ](https://kimi.ai) <<<===

===>>> [ chatgpt ](https://chatgpt.com) <<<===

===>>> [ grow.google ](https://grow.google/career-dreamer/home/)

===>>> [ aimode ](https://google.com/aimode)

===>>> [ aistudio ](http://aistudio.google.com) 

===>>> [ deepseek ](https://deepseek.com )

===>>> [ aixploria ](https://www.aixploria.com/en/)

===>>> [ perplexity ](https://perplexity.ai)

===>>> [ perplexity ](https://www.perplexity.ai/comet) ===>>> browser

===>>> [ youlearn ](https://youlearn.ai)

===>>> [ gemini ](https://gemini.google.com)

===>>> [ grok ](https://grok.com)

===>>> [ platform.openai ](https://platform.openai.com)

===>>> [ huggingface ](https://huggingface.co/spaces)

===>>> [ phind ](https://www.phind.com/)

===>>> [ Duck.ai ](Duck.ai) ===>>> https://duckduckgo.com/?q=DuckDuckGo+AI+Chat&ia=chat&duckai=1 <<<===

===>>> [ copilot.microsoft ](https://copilot.microsoft.com)

===>>> [ getonit ](https://www.getonit.ai/)

===>>> [ synth-inc ](https://github.com/synth-inc/onit)


==============================


تولید محتوای لینکدین:
redactai.io

تولید محتوا

undetectable.ai
===>>> humanizer.pro <<<===

سند PDF
[ PDF RESEARCH ](pdfsearch.io)


تولید اسلاید برای ارائه
[ PowerPoint ](pageon.ai)
[ slidesgpt ](slidesgpt.com)


==================================
PDF ===>>> chatpdf

PROMPT ===>> promptify.ai

LOGO ===>>> ailogomaker.io

ENGLISH LANGUAGE ===>>> pi.ai



==================================


https://answersai.com/

=================================

دیاگرام :

[ diagram ](http://app.eraser.io/)

=================================
## Mapify

 باخلاصه کردن یک کتاب  یا مقاله در چند دقیقه، زمینه مطالعه آسان حجم عظیمی از منابع را فراهم می کند. 
با دادن یا فایل به 
Mapify.so
 درخت محتوا و نقشه ذهنی آن را به دست آورید!

=================================

===>>> ninite

===>>> prezi.com

===>>> mapify.so

===>>> chat mind

===>>> gamma.app.com

=================================

## API

https://docanalyzer.ai/app/api

=================================


🔗 https://hix.ai

🔗 https://claude.ai

🔗 https://chatgpt.com

🔗 https://chat.qwen.ai

🔗 https://chat.mistral.ai

🔗 https://talkai.info/chat

🔗 https://julius.ai/models

🔗 https://deepai.org/chat

🔗 https://www.aichatting.net

🔗 https://chat.deepseek.com

🔗 https://aistudio.google.com

🔗 https://copilot.microsoft.com

🔗 https://gemini.google.com/app

🔗 https://notebooklm.google.com

🔗 https://overchat.ai/chat/best-free-ai-chat


=================================


پلتفرهای ایرانی :


gapgpt.app

hooshak.com

avalai.ir

==================================================

هوش مصنوعی :

===>>> https://mistral.ai


==================================================

هوش مصنوعی هایی که هر دانشجویی لازم داره :

https://scite.ai 

https://www.papergen.ai

https://outread.ai

https://storm.genie.stanford.edu

https://www.connectedpapers.com


==================================================

آنالیزهای هوش مصنوعی
https://artificialanalysis.ai/


==================================================


 افزونه همدست؛ دستیار فارسی هوش مصنوعی

https://chromewebstore.google.com/detail/hamdast-chatgpt-rtl-persi/lmfekemieicijlllcbbcfbeepmpfiedb

 ==================================================

--->>> ChatGPT ➜ تولید متن و پاسخ به سوالات


--->>> Google Bard ➜ جستجو و تولید محتوا با هوش مصنوعی


--->>> Notion AI ➜ مرتب‌سازی یادداشت‌ها و مدیریت کارها


--->>> Grammarly ➜ چک کردن گرامر و بهبود نوشتار

--->>> Canva Magic Write ➜ ایده‌پردازی برای محتوا


--->>> Hemingway ➜ ساده‌سازی متن و افزایش وضوح


--->>> Otter.ai ➜ تبدیل گفتار به نوشتار


--->>> Tome ➜ ساخت ارائه‌های هوش مصنوعی

--->>> Zapier ➜ خودکارسازی کارها


--->>> Lumen5 ➜ تبدیل متن به ویدیو


--->>> Pictory ➜ ساخت ویدیوهای کوتاه


--->>> DeepL ➜ ترجمه دقیق متن

--->>> Scribe AI ➜ ساخت راهنماها


--->>> DALL-E 3 ➜ تولید تصاویر


--->>> Copy ai ➜ تولید محتوای تبلیغاتی


--->>> Ecosia ➜ جستجوی درختی


===========================================================



هوش مصنوعی های مخصوص هر شغل...!

☑هوش مصنوعی فقط برای برنامه‌نویس‌ها نیست! الان برای هر شغلی ابزارهایی وجود داره که می‌تونن کارتو راحت‌تر کنن، سرعتتو بالا ببرن و حتی کیفیت خروجی کارت رو بالا ببرن.

1️⃣حسابدارها ← [Sheet AI] 
• محاسبات مالی و مدیریت درآمد 

2️⃣دانشجوها ← [Notion AI] 
• خلاصه‌نویسی و مدیریت دروس 

3️⃣وکلا ← [ChatGPT] 
• نوشتن قرارداد و مشاوره حقوقی 

4️⃣معمارها ← [Planner 5D] 
• طراحی سه‌بعدی و نقشه‌کشی 

5️⃣پزشکان ← [Symptoma AI] 
• تشخیص و کمک به درمان بیماری‌ها 

6️⃣طراحان داخلی ← [Home Styler] 
• چیدمان و مدل‌سازی دکوراسیون 

7️⃣معلم‌ها ← [Kahoot] 
• ساخت آزمون و آموزش تعاملی 

8️⃣گرافیست‌ها ← [Photopea] 
• ویرایش حرفه‌ای عکس‌ها 

===========================================================

⬅️ هوش مصنوعی تبدیل صدا به متن (کاملاً رایگان!)

اگه سر کلاس، جلسه یا حتی توی مکالمات روزمره‌تون نیاز دارین صدا ضبط کنین و متن کامل فارسیشو داشته باشین، این وب‌سایت عالیه!

👇
www.silenttype.litgrades.com

⬅️وارد سایت بشید
⬅️یه اکانت بسازید
⬅️ ایمیل تاییدو باز کنید و تاییدش کنین
⬅️ حالا می‌تونید مکالمات رو ضبط کنین و متن دقیقشو ببینید!

📌تو نسخه انگلیسی امکانات بیشتری مثل خلاصه‌سازی جلسه، ساخت کارت یادآوری و هایلایت نکات رو هم داره.
ولی توی زبان فارسی فعلاً می‌تونین متن رو بگیرین 

👈 هفته‌ای تا ۱۸۰ دقیقه رایگان!


==========================================

🔥دیگه لازم نیست واسه مشکلاتت کسی رو صدا کنی ؛ Ai حلش می‌کنه😮

📰یه ارور یا مشکل تو سیستم دیدی و نمی‌دونی چیکار کنی؟ نگران نباش. با این دو روش خیلی سریع و خودت‌کاره مشکل رو پیدا و رفع کن :

1️⃣راه اول Screen-share با Google AI Studio

☑️ وارد لینک زیر شو ، بخش Screen ، Screen share رو بزنی. صفحه‌ات برای هوش مصنوعی به‌اشتراک گذاشته میشه؛ اون همه‌چی رو می‌بینه و قدم‌به‌قدم راهنمایی می‌کنه. روی گوشی هم کار می‌کنه - راحت و مؤثر.

✅ https://aistudio.google.com/

2️⃣ارور رو کپی کن، تو ChatGPT پیست کن

☑️ هر جا اروری دیدی، روش کلیک کن ، Ctrl + C ، برو تو ChatGPT و Ctrl + V رو بزن و بنویس «این مشکل چیست و چطور رفع می‌شود؟»

=====================================


 نکات کلیدی هنگام استفاده از ChatGPT
1 سؤال دقیق بپرس:�هرچی سؤال‌ت واضح‌تر و جزئی‌تر باشه، جواب دقیق‌تر می‌گیری. مثلاً به‌جای “چطور تولید محتوا کنم؟” بپرس “چطور برای پیج اینستاگرام پوشاک محتوای تبلیغاتی بنویسم؟”
2 نقش بده به چت‌جی‌پی‌تی:�بگو “مثل یک متخصص بازاریابی عمل کن” یا “مثل یک نویسنده‌ی طنز جواب بده” — این باعث میشه لحن و نتیجه خیلی دقیق‌تر باشه.
3 بهش داده بده:�اطلاعات لازم رو بده تا خروجی بر اساس شرایط تو باشه (مثلاً نوع مخاطب، هدف محتوا، زبان برند و …).
4 ازش اصلاح و بازنویسی بخواه:�بگو “این متن رو رسمی‌تر کن” یا “نسخه خلاصه‌ترش رو بده” — هر بار بهترش کن.
5 بهش یاد بده:�می‌تونی نمونه‌ها یا سبک دلخواهت رو بهش بدی تا یاد بگیره و شبیه اون تولید کنه.
6 سؤال پشت‌سرهم بپرس:�نترس از ادامه دادن گفتگو. هر چی تعاملت بیشتر باشه، نتیجه دقیق‌تر میشه.
7 ازش برای ایده‌سازی استفاده کن، نه تصمیم نهایی:�خروجی‌ها رو الهام بگیر، اما حتماً با سلیقه و تجربه خودت ترکیب کن

=====================================


ساده‌ترین راه حمایت از من کلیک کردن روی ستاره (⭐) بالای همین صفحه است.

